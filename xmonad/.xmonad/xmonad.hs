...
myWorkspaces = [wsGEN, wsWRK, wsWRK2, wsSYS, wsMON, wsFLOAT, wsRW, wsTMP, wsOSS, wsBSA]
...
myBrowser           = "chromium" -- chrome with WS profile dirs
myBrowserClass      = "chromium"
myStatusBar         = "xmobar -x0 /home/taylor/.xmonad/xmobar.conf"
...
notesCommand         = "emacsclient -c  -F \"(quote (name . \\\"emacsNotes\\\"))\" /home/taylor/notes/master.org"
plexInfix           = "Plex"
plexResource        = "crx_fpniocchabmgenibceglhnfeimmdhdfm"
-- notesResource        = "crx_fpniocchabmgenibceglhnfeiaaaaaaa"
isPlex              = (resource =? plexResource)
isNotes              = (className =? "emacsNotes")

isConsole           = (className =? "TermScratchpad")
                    -- <&&> (stringProperty "WM_WINDOW_ROLE" =? "Scratchpad")
myConsole           = "urxvt -name TermScratchpad"
-- myConsole           = "urxvt -T console -p console --role=Scratchpad"
...
scratchpads =
    ...
    ,   (NS "emacsNotes"  notesCommand (className =? "emacsNotes") defaultFloating)
    -- ,   (NS "emacsNotes"  notesCommand isNotes defaultFloating)
    ...
    ,   (NS "console"  myConsole isConsole defaultFloating)
...
myKeys conf = let
  ...
  --, ("M-7"                    , addName "TESTING"                         $ runInTerm "-name glances" "glances" )
    ...
    , ("M-M1-p"                   , addName "Capture screen"                  $ spawn "screenshot" )
    , ("M-M1-S-p"                 , addName "Capture screen - area select"    $ spawn "screenshot area" )
    ...
    , ("M-n"                    , addName "NSP Notes"                     $ namedScratchpadAction scratchpads "emacsNotes")
    , ("M-g"                  , addName "Cancel submap"                   $ return ())
    ] ^++^
    ...
    , ("M-S-d"                    , addName "Duplicate w to all ws"           $ toggleCopyToAll)
    ...
    , ("M-s"                    , addName "Un-merge from sublayout"         $ withFocused (sendMessage . UnMerge))
    , ("M-S-s"                  , addName "Merge all into sublayout"        $ withFocused (sendMessage . MergeAll))
    ...
    , ("M-M1-'"                    , addName "Swap tab D"                      $ windows W.swapDown)
    , ("M-M1-;"                    , addName "Swap tab U"                      $ windows W.swapUp)
...
myStartupHook = do
    ...
    spawn "wpg -m"
    spawn "sh ~/.screenlayout/primary.sh"
    spawn "compton"
...
myManageHook =
    ...
    where
        manageSpecific = composeOne
            ...
            , className =? "emacsNotes" -?> doFullFloat
